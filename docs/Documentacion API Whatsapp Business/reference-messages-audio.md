...
``_Boolean_|  Boolean indicating if audio is a recording made with the WhatsApp client voice recording feature (`true`) or not (`false`).| `true`  
...
``_String_|  Media asset MIME type.| `audio/ogg; codecs=opus`  
