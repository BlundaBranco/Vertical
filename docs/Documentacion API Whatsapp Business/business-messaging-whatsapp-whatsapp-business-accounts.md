...
## Limitations
  ...
  * Meta Business Accounts are initially limited to 2 registered business phone numbers, but this limit can be increased to up to 20. See [Registered Number Limits](/documentation/business-messaging/whatsapp/business-phone-numbers/phone-numbers#registered-number-limits).
...
If you are going to be using Cloud API directly to send and receive messages, follow the steps in the [Cloud API Get Started](/documentation/business-messaging/whatsapp/get-started) documentation. Once you have completed these steps, you will have a test WABA and test business number, and have access to the **App Dashboard** > **WhatsApp** > **API Setup** panel.
...
Use the [GET /](/documentation/business-messaging/whatsapp/reference/whatsapp-business-account/whatsapp-business-account-api#Reading) endpoint to get data on a WABA. Use the `fields` parameter to request specific fields on a WABA, or omit it to get default fields returned by the endpoint.
...
Subscribe to the [account_update](/documentation/business-messaging/whatsapp/webhooks/reference/account_update) webhook to be notified of changes to a WhatsApp Business Account’s status, including changes due to policy and terms violations.
...
See our [WhatsApp Business Platform Policy Violations](/documentation/business-messaging/whatsapp/policy-enforcement-violations) document for a list of policy violations. If a restriction has been imposed, an **account_update** webhook will be triggered, describing the violation.

## Messaging On-Behalf-Of

The On-Behalf-Of WABA ownership model is deprecated is no longer possible. See [OBO model deprecation](/documentation/business-messaging/whatsapp/solution-providers/obo-model-deprecation) for details.
