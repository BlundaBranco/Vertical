...
Templates are WhatsApp Business Account assets that can be sent in template messages via Cloud API or Marketing Messages API for WhatsApp. Template messages are the only type of message that can be sent to WhatsApp users outside of a [customer service window](/documentation/business-messaging/whatsapp/messages/send-messages#customer-service-windows), so templates are commonly used when messaging users in bulk, or when you need to message a user, but no customer service window is open between you and the user.

## Creation

Use the [POST //message_templates](/documentation/business-messaging/whatsapp/reference/whatsapp-business-account/template-api#post-version-waba-id-message-templates) endpoint or [message templates panel⁠](https://business.facebook.com/latest/whatsapp_manager/message_templates) in WhatsApp Manager to create a template.
...
Each template must be categorized as **authentication** , **marketing** , or **utility**. Our [template categorization](/documentation/business-messaging/whatsapp/templates/template-categorization) document describes how to assign the proper category to a template, and what can happen if we determine that a template has been mis-categorized.

Note that template categories also factor into [pricing](/documentation/business-messaging/whatsapp/pricing).

### Components

Templates are made up of various text, media, and interactive UI components, which you define upon template creation. Our [template components document](/documentation/business-messaging/whatsapp/templates/components) describes all possible components and how to define them.

Since there are a lot of components to choose from, we have dedicated [authentication](/documentation/business-messaging/whatsapp/templates/authentication-templates/authentication-templates), [marketing](/documentation/business-messaging/whatsapp/templates/marketing-templates), and [utility](/documentation/business-messaging/whatsapp/templates/utility-templates/utility-templates) template documents and sub-documents, each with code examples that show how to create various templates with commonly used components.

### Languages

You must assign a [template language code](/documentation/business-messaging/whatsapp/templates/supported-languages) upon template creation. Template strings and variables are not translated by Meta, so you are responsible for supplying strings and example parameters in their appropriate language.
...
Template header components can display media assets. If you are creating a template with a media header, you must use the [Resumable Upload API](/docs/graph-api/guides/upload) to obtain an asset handle, and include this asset handle in your template creation request. The example asset will be reviewed as part of [template review](/documentation/business-messaging/whatsapp/templates/template-review).
...
See our [template review](/documentation/business-messaging/whatsapp/templates/template-review) document to learn more about the template review process, common rejection reasons, and what you can do if your template is rejected.

## Template status

Templates must have a status of `APPROVED` before they can be sent in template messages. A template’s status is initially set by the template review process, but can be changed to another value based on usage and [quality feedback](/documentation/business-messaging/whatsapp/templates/template-quality).

Template status changes are communicated via [message_template_status_update](/documentation/business-messaging/whatsapp/webhooks/reference/message_template_status_update) webhooks, but you can use the [GET /](/documentation/business-messaging/whatsapp/reference/whatsapp-business-account/template-api#fields) endpoint and request the `status` field to get the status of a template at any time.
...
See the [GET /](/documentation/business-messaging/whatsapp/reference/whatsapp-business-account/template-api#fields) endpoint reference for a list of all possible status values and what they mean.
...
The [Manage templates⁠](https://business.facebook.com/latest/whatsapp_manager/message_templates) panel in WhatsApp Manager also displays template statuses, and appends quality ratings for approved (`active`) templates:
  ...
  * **Paused** : The template has been paused due to recurring negative feedback from customers, or low read-rates. Message templates with this status cannot be sent to customers. See [Template Pausing](/documentation/business-messaging/whatsapp/templates/template-pausing).
...
If a parent business portfolio is unverified, each of its WhatsApp Business Accounts is limited to 250 message templates. However, if the portfolio is [verified⁠](https://www.facebook.com/business/help/1095661473946872), and at least one of its WhatsApp Business Accounts has a business phone number with an approved [display name](/documentation/business-messaging/whatsapp/display-names), each of its WhatsApp Business Accounts can have up to 6,000 templates.
...
Additionally, there are limits on the number of templates you can send, as well as processes that can affect template delivery:
  ...
  * [Messaging limits](/documentation/business-messaging/whatsapp/messaging-limits) — A limit on the number of message templates you can send outside of customer service windows.
  * [Template pacing](/documentation/business-messaging/whatsapp/templates/template-pacing) — A process that allows time for WhatsApp users to provide feedback on message templates.
  * [Template pausing](/documentation/business-messaging/whatsapp/templates/template-pausing) — A process that can temporarily pause message templates that have received poor feedback.
  * [Per-user marketing template message limits](/documentation/business-messaging/whatsapp/templates/marketing-templates/per-user-limits) — A process that limits the number of marketing message templates a given WhatsApp user may receive from any business.
...
See our [time-to-live](/documentation/business-messaging/whatsapp/templates/time-to-live) document for more information.
...
See our [template quality rating](/documentation/business-messaging/whatsapp/templates/template-quality) document for more information about quality ratings, how they can affect a template’s status, and how you can be notified of changes to template quality scores.
...
See our [template management](/documentation/business-messaging/whatsapp/templates/template-management) document for a list of endpoints commonly used for getting, updating, and deleting templates.
