...
Quality ratings factor into [template pacing](/documentation/business-messaging/whatsapp/templates/template-pacing) and [template pausing](/documentation/business-messaging/whatsapp/templates/template-pausing), which can affect template delivery. If a template continuously receives negative feedback or low engagement, it can eventually affect the template’s status. If the status changes to anything other than `APPROVED`, the template cannot be sent in template messages unless its `APPROVED` status is restored.

## Get template quality rating via API

Use the [GET /](/documentation/business-messaging/whatsapp/reference/whatsapp-business-account/template-api#Reading) endpoint and request the `quality_score` field to get a template’s quality score.
