...
Use the value of `intentUrl` in the response and pass it as the `upi_intent_link` while setting up the [`parameters` object to send for the order details message using the WhatsApp API](/documentation/business-messaging/whatsapp/payments/payments-in/upi-intent/dynamic-vpa#paymentsettingsobject).
...
Utilize the `order_status` retrieved from the response above and transmit the corresponding [status message](/docs/whatsapp/cloud-api/payments-api/upi#step-7--update-order-status) via WhatsApp API.
