...
Session Initiation Protocol ([SIP⁠](https://l.facebook.com/l.php?u=https%3A%2F%2Fdatatracker.ietf.org%2Fdoc%2Fhtml%2Frfc3261&h=AT6qNb5TO88Qwj12IY3povJZ2d3RKTKwmrG_jKgVojy4nzPEZnYjYRebfm1eFdjtZnso6fMNI-VjzAe1rFWwDB8Nl0WPN05ZZCKuW5dxrE1wKnd8qn0DJy7M_ZSC0Hj0u7v6tL5WlZ6pWw9jrdKTRA)) is a signaling protocol used for initiating, maintaining, modifying, and terminating real-time communication sessions between two or more endpoints.
...
Before you get started with SIP call signaling, confirm the following:
  ...
  * You meet overall [calling pre-requisites](/documentation/business-messaging/whatsapp/calling#step-1--prerequisites)
  * Your app has messaging permissions for the business phone number you want to enable SIP for. 
    ...
    * Double confirm this by using [health status API](/documentation/business-messaging/whatsapp/support/health-status) with `PHONE_NUMBER_ID`
  * Your app mode is “Live”, not “Development”.
  * You have a standards compliant third party SIP server that supports [TLS](/documentation/business-messaging/whatsapp/calling/sip#security) transport and digest authentication

See [Signaling and media possible configurations](/documentation/business-messaging/whatsapp/calling#signaling-and-media-possible-configurations) for more info

## Calling flows using SIP

Before you start, make sure you have [enabled and configured SIP on the business phone number](/documentation/business-messaging/whatsapp/calling/sip#configure-update-sip-settings-on-business-phone-number). Meta generates a unique SIP user password for each business phone number + app combination. You will need this information and can retrieve it by using the [get Call Settings endpoint.](/documentation/business-messaging/whatsapp/calling/sip#get-phone-number-calling-settings--sip-)
...
### Security
  ...
  * For business initiated calls, SIP invite from your SIP server will be challenged using digest auth. See [business-initiated calls](/documentation/business-messaging/whatsapp/calling/sip#business-initiated-calls) for more details
  * For user initiated calls, it is highly recommended that you challenge SIP INVITE request from Meta, to use digest auth for more security. See [user-initiated calls](/documentation/business-messaging/whatsapp/calling/sip#user-initiated-calls) for more details
...
In this case, you can alter the certificate to match your hostname or [change your configured SIP server hostname](/documentation/business-messaging/whatsapp/calling/sip#configure-update-sip-settings-on-business-phone-number) to match your certificate
...
#### Prerequisites
  ...
  * You have the required call permission approval from the WhatsApp user 
    * [Learn how to obtain user calling permissions](/documentation/business-messaging/whatsapp/calling/user-call-permissions)
  * [Retrieve Meta generated SIP password](/documentation/business-messaging/whatsapp/calling/sip#include-sip-user-password) and configure it on your SIP server, so it can respond to digest authentication challenge from Meta SIP servers

#### Calling flow

  1. Send an initial [SIP INVITE](/documentation/business-messaging/whatsapp/calling/sip#business-initiated-calls--with-webrtc-media-) to our servers. Our SIP domain is wa.meta.vc. To initiate a call to WhatsApp user with phone number 11234567890, the SIP request URI should be ‘sip:+11234567890@wa.meta.vc;transport=tls’ 
     * This request will fail with an “SIP 407 Proxy Authentication required” message.
  2. Send a 2nd [SIP INVITE](/documentation/business-messaging/whatsapp/calling/sip#business-initiated-calls--with-webrtc-media-) with proper Authorization header as per [RFC 3261⁠](https://l.facebook.com/l.php?u=https%3A%2F%2Fdatatracker.ietf.org%2Fdoc%2Fhtml%2Frfc3261%23section-22&h=AT6qNb5TO88Qwj12IY3povJZ2d3RKTKwmrG_jKgVojy4nzPEZnYjYRebfm1eFdjtZnso6fMNI-VjzAe1rFWwDB8Nl0WPN05ZZCKuW5dxrE1wKnd8qn0DJy7M_ZSC0Hj0u7v6tL5WlZ6pWw9jrdKTRA). 
     * The Authorization field’s username attribute must match the from header’s user name which is the business phone number
     * The password is generated by Meta and you can retrieve it using [get Call Settings endpoint](/documentation/business-messaging/whatsapp/calling/sip#get-phone-number-calling-settings--sip-)
  ...
  3. Send the [SIP INVITE](/documentation/business-messaging/whatsapp/calling/sip#business-initiated-calls--with-webrtc-media-) to the WhatsApp user number you want to call.

### User-initiated calls

#### Prerequisites

  * If you plan to use SIP Digest Auth, [retrieve Meta generated SIP password](/documentation/business-messaging/whatsapp/calling/sip#include-sip-user-password) and configure it on your SIP server, so it can respond to digest authentication challenge from Meta SIP servers
...
#### Calling flow
  ...
  2. If the business phone number is SIP enabled, Meta will send an [SIP INVITE](/documentation/business-messaging/whatsapp/calling/sip#user-initiated-calls--with-webrtc-media-) to the SIP server [configured on the business phone number](/documentation/business-messaging/whatsapp/calling/sip#configure-update-sip-settings-on-business-phone-number)
  3. You respond with [SIP digest auth challenge](/documentation/business-messaging/whatsapp/calling/sip#user-initiated-calls-with-digest-auth--with-sdes-media-) (recommended) or [SIP OK](/documentation/business-messaging/whatsapp/calling/sip#user-initiated-calls--with-webrtc-media-) and pass in an SDP answer

If you are not receiving SIP INVITE from Meta, refer to [SIP specific FAQ](/documentation/business-messaging/whatsapp/calling/faq#session-initiation-protocol--sip--faq) to troubleshoot further

[View sample SIP requests](/documentation/business-messaging/whatsapp/calling/sip#sample-sip-requests)

[Learn more about Session Description Protocol (SDP)⁠](https://l.facebook.com/l.php?u=https%3A%2F%2Fwww.rfc-editor.org%2Frfc%2Frfc8866.html&h=AT6qNb5TO88Qwj12IY3povJZ2d3RKTKwmrG_jKgVojy4nzPEZnYjYRebfm1eFdjtZnso6fMNI-VjzAe1rFWwDB8Nl0WPN05ZZCKuW5dxrE1wKnd8qn0DJy7M_ZSC0Hj0u7v6tL5WlZ6pWw9jrdKTRA)
...
Header name |  Metadata |  Description   
---|---|---  
x-wa-meta-cta-payload| Optional; String| Present when user-initiates a call from call button that has business specified payload. [Learn more](/documentation/business-messaging/whatsapp/calling/call-button-messages-deep-links#send-interactive-message-with-a-whatsapp-call-button)  
x-wa-meta-deeplink-payload| Optional; String| Present when user-initiates a call from call deeplink that has business specified payload. [Learn more](/documentation/business-messaging/whatsapp/calling/call-button-messages-deep-links#send-payload-data-in-call-deeplink)  
...
---|---|---  
``_Integer_| **Required**  
The business phone number for which you are updating Calling API settings.[Learn more about formatting phone numbers in Cloud API](/documentation/business-messaging/whatsapp/business-phone-numbers/phone-numbers)| `+12784358810`  
...
`servers` _String_| **Optional**  
The SIP server routing configuration.Each phone number can have only one SIP server configured per application. The servers is an array to be futureproof. It also makes the POST payload schema be consistent with GET payload schema because you can have multiple apps each with their own SIP server. In the GET payload, if you see multiple SIP servers, it means you’ve used the POST API with different access tokens that belong to different apps.The associated app is extracted from the access token used to make the API call.Since a business phone number may be used with multiple applications, each number can be serviced by multiple SIP servers.To delete a previously configured SIP server, pass an empty array to this field. If you still see some servers remaining after you clear, those servers may belong to different apps, so you need to use the corresponding access tokens to clear them.Note that at-least 1 SIP server of any app must exist when SIP status is ENABLED. To clear servers for all applications being used with a business phone number, the SIP status should be DISABLED.`hostname` — (_String_) **Required** The host name of the SIP server.Requests must use TLS.`port` — (_String_) **Required** The port within your SIP server that will accept requests.Requests must use TLS.Default port is 5061`request_uri_user_params` — (_String_) **Optional** An optional field for passing any parameters you want included in the user portion of the request URI used in our SIP INVITE to your SIP server.Max key/value size is 128 characters.An example use case would be Trunk Groups ([RFC 4904⁠](https://l.facebook.com/l.php?u=https%3A%2F%2Fdatatracker.ietf.org%2Fdoc%2Fhtml%2Frfc4904&h=AT6qNb5TO88Qwj12IY3povJZ2d3RKTKwmrG_jKgVojy4nzPEZnYjYRebfm1eFdjtZnso6fMNI-VjzAe1rFWwDB8Nl0WPN05ZZCKuW5dxrE1wKnd8qn0DJy7M_ZSC0Hj0u7v6tL5WlZ6pWw9jrdKTRA))

  * sip:+1234567890@sip.example.com
  * tgrp=wacall
  * trunk-context=byoc.example.com

This example has two user parameters for tgrp, and trunk-context.The effective SIP request URI line for this would be `sip:+1234567890;tgrp=wacall;trunk-context=byoc.example.com@sip.example.com`  
...
---|---|---  
``_Integer_| **Required**  
The business phone number for which you are retrieving Calling API settings.[Learn more about formatting phone numbers in Cloud API](/documentation/business-messaging/whatsapp/business-phone-numbers/phone-numbers)| `+12784358810`  
...
By default, the response body does not include the Meta generated SIP password. To include the password in the respons body, add the optional SIP credentials query parameter in the GET request:
...
To make Meta generate a new SIP password, you’d need to disable SIP, delete SIP server and add your SIP server back.
  ...
  * [Fetch your SIP configuration with password](/documentation/business-messaging/whatsapp/calling/sip#include-sip-user-password) to view your current password for your reference
  ...
  * [Fetch your SIP configuration with password](/documentation/business-messaging/whatsapp/calling/sip#include-sip-user-password) to note the new password
...
#### 407 response from Meta
    ...
    Call-ID: dc2c5b33-1b81-43ee-9213-afb56f4e56ba
    From: ;tag=Kc9QZg4496maQ
    To: ;tag=z9hG4bK0da2.36614b8977461b486ceabc004c723476.0
    CSeq: 96743476 INVITE
...
#### Example error response
    ...
    SIP/2.0 403 SIP server wa.meta.vc from INVITE does not match any SIP server configured for phone number id {ID}
            ...
            From: ;tag=3a63b370-a697-4a5a-82b4-e8105e23f176
            To: ;tag=e0d30a05-657b-47ec-a668-e05ca79f9f05
            CSeq: 15659 INVITE
...
#### SIP BYE
    ...
    Route: 
    Max-Forwards: 69
    From: ;tag=NcKQ6mtDKSDQB
    To: "5559800000693" ;tag=92a01092-ee78-4870-865f-bc176203a6bd
...
#### SIP INVITE (with SDES)
    ...
    Record-Route: 
    CSeq: 2 INVITE
    From: "12145551869" ;tag=28460006_c3356d0b_5cdada8c-cbf0-4369-b02d-cc97d3c36f2b
    To: 
    Max-Forwards: 66
    P-Asserted-Identity: 
    Min-SE: 120
...
#### SIP INVITE
    ...
    Max-Forwards: 70
    From: "12195550714" ;tag=bbf1ad6e-79bb-4d9c-8a2c-094168a10bea
    To: 
    Contact: ;isfocus
...
#### SIP INVITE (with SDES)
    ...
    INVITE sip:+12145551869@meta-voip.example.com;transport=tls SIP/2.0
                ...
                Max-Forwards: 69
                From: "12195550714" ;tag=8a0f7e65-6e9e-4801-bf92-85c3ef2485d9
                To: 
                Contact: ;isfocus
...
#### SIP OK (with SDES)
    ...
    SIP/2.0 200 OK
                ...
                Call-ID: outgoing:wacid.HBgLMTIxOTU1NTA3MTQVAgASGCA4QkY1MTJCQkNFNTgxMEVFRERFRTUzNTFERkE1MDU0MhwYCzEyMTQ1NTUxODY5FQIAAA
                From: "12195550714" ;tag=8a0f7e65-6e9e-4801-bf92-85c3ef2485d9
                To: ;tag=66596922_c3356d0b_fee164be-566a-4679-a80d-5bfdf1d0aa9e
...
Meta SIP server supports digest auth for user initiated calls. Your SIP server should respond with digest auth challenge and Meta will resend the SIP INVITE with challenge response. The username used for digest auth is the (normalized) business phone number and the password is generated by Meta and retrievable using the [get Call settings endpoint](/documentation/business-messaging/whatsapp/calling/sip#include-sip-user-password).
...
#### First INVITE request from Meta
    ...
    Max-Forwards: 69
    From: "12195550714" ;tag=ece2da15-39e7-4983-ac65-e312f325d94a
    To: 
    Contact: ;isfocus
...
#### 407 Response from partner SIP server
    ...
    Call-ID: outgoing:wacid.HBgLMTIxOTU1NTA3MTQVAgASGCA2MUI2QUY0MDRCMTUyOTM4QkE5ODEwN0ZGQTAwODkxORwYCzEyMTQ1NTUxODY5FQIAFRoA
    From: "12195550714" ;tag=ece2da15-39e7-4983-ac65-e312f325d94a
    To: ;tag=45065608_c3356d0b_16001fd8-76d2-45f0-bb35-e0441d6dc4a2
...
#### SIP OK from partner SIP server
    ...
    Call-ID: outgoing:wacid.HBgLMTIxOTU1NTA3MTQVAgASGCA2MUI2QUY0MDRCMTUyOTM4QkE5ODEwN0ZGQTAwODkxORwYCzEyMTQ1NTUxODY5FQIAFRoA
    From: "12195550714" ;tag=ece2da15-39e7-4983-ac65-e312f325d94a
    To: ;tag=29360930_c3356d0b_4933dc58-f035-4597-b075-04b19e552329
...
---|---|---  
`srtp_key_exchange_protocol` _String_| **Optional**  
Enable or disable use of SRTP key exchange protocol.Possible values are `SDES` and `DTLS`.Default is `DTLS`.Note: Meta still expects the business side to send the maiden SRTP packet for both user and business initiated calls| `“SDES”`  
...
Parameter |  Description |  Sample Value   
---|---|---  
`srtp_key_exchange_protocol` _String_|  The type of SRTP key exchange protocol configured for the business phone number queriedPossible values are `SDES` and `DTLS`.Default is `DTLS`.**Note: If this field has not been explicitly set, it will not be returned.**| `“SDES”`  
...
The IP addresses used for SIP configuration are the same as those listed for the Webhooks in the [Cloud API Webhooks IP Addresses section](/documentation/business-messaging/whatsapp/webhooks/overview#ip-addresses).
...
Refer to [SIP FAQ](/documentation/business-messaging/whatsapp/calling/faq#session-initiation-protocol--sip--faq) for additional SIP specific questions and answers and [SIP Errors](/documentation/business-messaging/whatsapp/calling/troubleshooting#sip-errors) for SIP specific errors and solutions
