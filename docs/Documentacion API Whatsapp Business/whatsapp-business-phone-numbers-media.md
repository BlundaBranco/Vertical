...
Some of the API requests described in this document require a media ID. Media IDs are returned by the API when uploading media, and are included in incoming media messages webhooks ([image messages](/documentation/business-messaging/whatsapp/webhooks/reference/messages/image), [video messages](/documentation/business-messaging/whatsapp/webhooks/reference/messages/video), etc.)
...
---|---  
`/PHONE_NUMBER_ID/media`(See [Get Phone Number ID](/documentation/business-messaging/whatsapp/business-phone-numbers/phone-numbers#get-all-phone-numbers))| Developers can authenticate their API calls with the access token generated in the **App Dashboard** > **WhatsApp** > **API Setup**.Solution Partners must authenticate themselves with an access token with the `whatsapp_business_messaging` permission.  
...
### Request
    ...
    curl 'https://graph.facebook.com///media' \  
...
You can query a media ID directly to get a media URL, which you can then query directly with your access token to download the media asset. Note that incoming media messages webhooks ([image messages](/documentation/business-messaging/whatsapp/webhooks/reference/messages/image), [video messages](/documentation/business-messaging/whatsapp/webhooks/reference/messages/video), and so on) include the media URL, which is assigned to the `url` property.
...
### Request syntax
    ...
    curl 'https://graph.facebook.com//?phone_number_id=' \  
    -H 'Authorization: Bearer EAAJB'  
...
Use the **DELETE / ** endpoint to delete a media asset.

### Request syntax
    
    
    curl -X DELETE 'https://graph.facebook.com//?phone_number_id=' \  
    -H 'Authorization: Bearer EAAJB...'  
...
If the download attempt fails, you will receive a `404 Not Found` response code. In that case, try to get a new media URL and download it again. If doing so doesn’t resolve the issue, renew your access token and attempt to download the media asset again.
...
WebP images can only be sent in [sticker messages](/documentation/business-messaging/whatsapp/messages/sticker-messages).
...
The maximum supported file size for media messages on Cloud API is 100MB. In the event the customer sends a file that is greater than 100MB, you will receive a webhook with error code [131052](/documentation/business-messaging/whatsapp/support/error-codes#other-errors) and `title`:
